...
// Cyber Effects Class - Gelbe Version für Dev Website
class CyberEffects {
    constructor() {
    ...
    init() {
        console.log('⚡ Initializing Cyber Effects...');
    ...
    delayedInit() {
        setTimeout(() => {
            ...
            console.log('✅ Cyber Effects fully initialized');
        }, 100);
    }

    addCyberStyles() {
        // Add CSS keyframes if they don't exist
        if (!document.querySelector('#cyber-effects-styles')) {
            const style = document.createElement('style');
            style.id = 'cyber-effects-styles';
            style.textContent = `
    ...
    createCodeRainContainer() {
        ...
        const existing = document.getElementById('cyberCodeRain');
        if (existing) {
            existing.remove();
        }

        const codeRain = document.createElement('div');
        codeRain.id = 'cyberCodeRain';
        codeRain.className = 'cyber-code-rain';
        ...
        console.log('✅ Cyber code rain container created');
    }
    ...
    generateCodeRain() {
        if (!this.codeRainContainer) {
            console.error('❌ Cyber code rain container not found');
            return;
        }
        ...
        for (let i = 0; i < columns; i++) {
            const column = document.createElement('div');
            column.className = 'cyber-code-column';
            column.style.cssText = `
                ...
                animation: cyber-code-fall ${3 + Math.random() * 3}s linear infinite;
        ...
        console.log('✅ Cyber code rain generated with', columns, 'columns');
    }

    setupCyberSwitch() {
        // Try multiple selectors to find the cyber switch
        ...
        // Force apply styles to make sure they stick
        this.forceApplyStyles(cyberSwitch);

        // Remove existing event listeners by cloning
        const newButton = cyberSwitch.cloneNode(true);
        cyberSwitch.parentNode.replaceChild(newButton, cyberSwitch);

        // Force apply styles again after cloning
        this.forceApplyStyles(newButton);
        ...
        console.log('✅ Cyber switch event listener attached');
    }

    forceApplyStyles(element) {
        // Force apply inline styles to override any CSS conflicts
        element.style.cssText = `
            ...
            background: linear-gradient(135deg, rgba(255, 193, 7, 0.15), rgba(255, 235, 59, 0.15), rgba(255, 193, 7, 0.15)) !important;
        ...
        element.addEventListener('mouseenter', () => {
            ...
            element.style.textShadow = '0 0 15px rgba(255, 193, 7, 1), 0 0 30px rgba(255, 235, 59, 0.8)';
            element.style.boxShadow = '0 0 40px rgba(255, 193, 7, 0.7), 0 12px 25px rgba(255, 193, 7, 0.4)';
        });
        ...
        element.addEventListener('mouseleave', () => {
            ...
            element.style.boxShadow = '0 0 20px rgba(255, 193, 7, 0.3)';
        });
    }
    ...
    createCircuitEffect() {
        ...
        circuit.style.cssText = `
            ...
            opacity: 0;
            animation: cyber-circuit-fade 2s ease-in-out;
        `;
        ...
        console.log('⚡ Cyber circuit effect created');
    }
    ...
    createDataStream() {
        ...
        stream.style.cssText = `
            ...
            z-index: 9999;
            pointer-events: none;
            animation: cyber-data-pulse 1.5s ease-in-out;
            font-weight: bold;
            letter-spacing: 2px;
        `;
        ...
        console.log('📡 Cyber data stream created');
    }
...
// Initialize cyber effects when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    console.log('⚡ DOM Content Loaded - Initializing Cyber Effects...');
    ...
    window.addEventListener('resize', () => {
        if (window.cyberEffects && window.cyberEffects.isInitialized) {
            window.cyberEffects.handleResize();
        }
    });

    console.log('✅ Cyber effects initialized successfully!');
});
